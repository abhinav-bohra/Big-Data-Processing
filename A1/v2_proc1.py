...
# python3.10 assignment-1-18CS30049.py 20_newsgroups <# processes> <value of n for n-gram> <value of k>
# python3.10 assignment-1-18CS30049.py 20_newsgroups 10 4 10
#-------------------------------------------------------------------------------------------------------------------
...
def getClass_ngrams(docs_map, n):
...
if __name__ == '__main__':
    ...
    #------------------------------------------------------------
    # STEP 2: CALCULATE N-GRAMS OF EACH DOCUMENT (MULTIPROCESSING)
    #------------------------------------------------------------
    results_class_ngrams = [None]*(num_processes)
    chunk_size = math.ceil(len(docs_map)/num_processes)    
    pool = multiprocessing.Pool(processes=num_processes)
    args_list = [(docs_map[i*chunk_size: (i+1)*chunk_size], n) for i in range(num_processes)]
    results_class_ngrams = pool.starmap(getClass_ngrams, args_list)
